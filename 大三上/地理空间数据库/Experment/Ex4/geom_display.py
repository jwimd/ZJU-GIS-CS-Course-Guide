    ...
    try:
        if type=="choroplethMap":
            if not result[0].has_key('value'):
                raise Exception('无对应的value列,results的关系类型为（gid，name，value,geom）')
            elif not result[0].has_key('geom'):
                raise Exception('无对应的geom列,results的关系类型为（gid，name，value,geom）')
            elif not result[0].has_key('name'):
                raise Exception('无对应的name列,results的关系类型为（gid，name，value,geom）')
            elif not result[0].has_key('gid'):
                raise Exception('无对应的gid列,results的关系类型为（gid，name，value,geom）')
            else:
                for row in result:
                    featureCollection.addFeature(Feature(row['geom'], row['name'],row['gid'],index,num=row['value']))
        elif type=="":
            if not result[0].has_key('gid'):
                raise Exception('无对应的gid列,results的关系类型为（gid，geom，name）')
            elif not result[0].has_key('geom'):
                raise Exception('无对应的geom列,results的关系类型为（gid，geom，name）')
            elif not result[0].has_key('name'):
                raise Exception('无对应的name列,results的关系类型为（gid，geom，name）')
            else:
                for row in result:
            ...
            if not result[0].has_key('gid'):
                raise Exception('无对应的gid列,results的关系类型为（gid，geom，name）或者（gid，geom，name，value）')
            elif not result[0].has_key('geom'):
                raise Exception('无对应的geom列,results的关系类型为（gid，geom，name）或者（gid，geom，name，value）')
            elif not result[0].has_key('name'):
                raise Exception('无对应的name列,results的关系类型为（gid，geom，name）或者（gid，geom，name，value）')
            else:
